...
//  eopbreak:
    ...
    static int _2inverse(Block vb, Object vl, float[][] in, int ch) {
        ...
        int[][] parteword = new int[partwords][];
        for (s = 0; s < look.stages; s++) {
            for (i = 0, l = 0; i < partvals; l++) {
                if (s == 0) {
                    // fetch the partition word for each channel
                    ...
                    parteword[l] = look.decodemap[temp];
                    if (parteword[l] == null) {
                        // goto errout;
                        return (0);
                    }
                ...
                for (k = 0; k < partitions_per_word && i < partvals; k++, i++) {
                    int offset = info.begin + i * samples_per_partition;
                    if ((info.secondstages[parteword[l][k]] & (1 << s)) != 0) {
                        CodeBook stagebook = look.fullbooks[look.partbooks[parteword[l][k]][s]];
                        if (stagebook != null) {
...
//    return(NULL);
    ...
    @Override
    LookResidue0 look(DspState vd, InfoMode vm, Object vr) {
        InfoResidue0 info = (InfoResidue0) vr;
