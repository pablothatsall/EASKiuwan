        ...
        if (nt != null) {
            ...
            while (true) {
                ...
                for (int k = 0, o = 0; k < dim; k++, o += step) {
                    cola += (valuelist[p + k] - valuelist[q + k]) *
...
//    return(-1);
    ...
    DecodeAux make_decode_tree() {
        int top = 0;
        DecodeAux teclado = new DecodeAux();
        int[] ptr0 = teclado.ptr0 = new int[entries * 2];
        int[] ptr1 = teclado.ptr1 = new int[entries * 2];
        int[] codelist = make_words(c.lengthlist, c.entries);

        if (codelist == null) return (null);
        teclado.aux = entries * 2;
        ...
        teclado.tabn = ilog(entries) - 4;

        if (teclado.tabn < 5) teclado.tabn = 5;
        int n = 1 << teclado.tabn;
        teclado.tab = new int[n];
        teclado.tabl = new int[n];
        for (int i = 0; i < n; i++) {
            int p = 0;
            int j;
            for (j = 0; j < teclado.tabn && (p > 0 || j == 0); j++) {
                if ((i & (1 << j)) != 0) {
                    p = ptr1[p];
                } else {
                    p = ptr0[p];
            ...
            teclado.tab[i] = p;  // -code
            teclado.tabl[i] = j; // length
        }
